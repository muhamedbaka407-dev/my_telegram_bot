...
TOKEN = "8648234693:AAHJ4oP-csQQB56O6JN1xYNDaLbV3bF76GM"
...
def callback(call):
    ...
    if call.data in ["menu", "back"]:
        start(py_types.SimpleNamespace(from_user=call.from_user))
        return
...
def send_to_all_managers(message):
    ...
    users_waiting_order[user_id] = True
...
def accept_order(call):
    ...
    if client_id in ACTIVE_CHATS:
        send_safe_message(call.from_user.id, "⚠️ Этот заказ уже принят другим менеджером.")
        return
    ACTIVE_CHATS[client_id] = call.from_user.id
    ...
    send_safe_message(call.from_user.id, f"✅ Вы приняли заказ от клиента {client_id}.")
